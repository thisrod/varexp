...
whence
$$|〈β|α〉|²=e^{-|β-α|²}.$$
Their projector follows
$$\int |α〉〈α|\,d²α=π.$$
For an orthonormal basis, the right hand side would be 1 instead of $π$.  It follows that the operator $\G$ is onto: let $|ψ〉$ any ket, and set $f₀(α*)=e^{½|α|²}〈α|ψ〉$.  Then
$$\G f₀={1\over π}\int |α〉〈α|ψ〉\,d²α=|ψ〉.$$
Furthermore, coherent states have the property that $e^{½|α|²}|α〉$ is an entire function of $α$, so $f₀$ is an entire function.  Glauber showed that it is the unique entire solution to $\G f₀=|ψ〉$.  Further properties of the coherent states will be stated as required.  Glauber's summary of them is paraphrased in every quantum optics textbook ever written.

Obviously, $\G$ is not one to one, because the images of the entire functions already fill ket space.
...
Given a proposed change $dz$, we can find the effect on the condition number.  However, it would be simpler to constrain the smallest singular value, and likely just as effective.

Let 
$$|A(α)〉=(|α₁〉 |α₂〉 \cdots |α_R〉)=(\cdots e^{-½|α_r|²+α_ra†}\cdots)|0〉.$$
This is not an analytic function, so there is no Cauchy derivative such that $|A(z+h)〉=|A(z)〉+|DA(z)〉h+O(h²)$.  There are, however, Wirtinger derivatives such that $|A(z+h)〉=|A(z)〉+|DA(z)〉h+|CA(z)〉h†$.  As a point of notation, let $〈A(z+h)|=〈A(z)|+h†〈CA(z)|+h〈DA(z)|$, so that $〈CA(z)|$ is the conjugate bra to $|DA(z)〉$.  Let $|DA(z)〉=(|∂₁A(z)〉\cdots |∂_RA(z)〉)$, and $|CA(z)〉=(|∂*₁A(z)〉\cdots |∂*_RA(z)〉)$.  Because $|A〉$ is a matrix valued function, these are also matrices, with $|DA〉$ and $|CA〉$ being higher rank tensors.  I hope the product notation is not too confusing.  The Wirtinger derivatives are
$$|∂_rA(α)〉=\pmatrix{0 & … & 0 & (-{α*_r\over2}+a†)|α_r〉& 0 & … & 0},$$
and
$$|∂*_rA(α)〉=\pmatrix{0 & … & 0 & -{α*_r\over2}|α_r〉& 0 & … & 0}.$$

To find the derivative of the smallest singular value $σ_R(α)$, consider the matrix
$$M(α)=\pmatrix{0 & |A(α)〉\cr 〈A(α)| & 0}.$$
Here, $|A(α)〉$ maps weight vectors in $C^R$ to kets, while $〈A(α)|$ maps kets back to column vectors of inner products.  The operation of the zeros is obvious; formally, they need to be a row of null kets and a column of null bras.  The matrix operates on vectors of a ket followed by $R$ complex numbers.  Let $|A(α)〉=|U〉SV†$ be a singular value decomposition, whence $〈A(α)|=VS〈U|$.  The vector
$$x={1\over \sqrt2}\pmatrix{|u_R〉 \cr v_R}$$
is a normalised eigenvector  of $M$, belonging to the eigenvalue $σ_R(α)$.  From first-order pertubation theory, the derivative of this eigenvalue with respect to $α_r$ is
$$∂_r σ_R=x†∂_rMx\qquad ∂*_r σ_R=x†∂*_rMx.$$
Then 
$$\eqalign{∂_r σ_R(α)&=x†∂_rM(α)x\cr
	&=x†\pmatrix{0 & |∂_rA(α)〉\cr 〈∂_rA(α)| & 0}x \cr
	&=½\pmatrix{〈u_R| & v†_r}
		\pmatrix{0 & \matrix{0& (-{α*_r\over2}+a†)|α_r〉&0}\cr 
			\matrix{0\cr -{α_r\over2}〈α_r|\cr 0}& }
		\pmatrix{|u_R〉 \cr v_R} \cr
	&=-½\Re(α*_rv_R^r〈u_R|α_r〉)+½v_R^r〈u_R|a†|α_r〉,
}$$
and similarly
$$∂*_r σ_R(α)=-½\Re(v_R^{r\ast}α_r〈α_r|u_R〉)+½v_R^{r\ast}〈α_r|a|u_R〉.$$
More simply, if $α=x+iy$,
$${∂σ_R\over ∂x} = ∂_r σ_R+∂*_r σ_R=\Re(v_R^r〈u_R|(a†-α*_r)|α_r〉),$$
$${∂σ_R\over ∂y} = i∂_r σ_R-i∂*_r σ_R=-\Im(v_R^r〈u_R|a†|α_r〉).$$
