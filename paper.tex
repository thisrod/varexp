...
% It follows that $\G$ is onto: let $|ψ〉$ any ket, and set $f₀(α*)=π^{-½}e^{½|α|²}〈α|ψ〉$.  Then
% $$\G f₀={1\over π}\int |α〉〈α|ψ〉\,d²α=|ψ〉.$$
% Furthermore, coherent states have the property that $e^{½|α|²}|α〉$ is an entire function of $α$ (clarify), so $f₀$ is an entire function.  Glauber showed that it is the unique entire solution to $\G f₀=|ψ〉$.
...
This all seems wrong, because the incomplete gamma function varies the wrong way with $n$.  It becomes small as $n→0$.
...
$$\vec g=\pmatrix{\langle u₁,g\rangle & ⋯ & \langle u_R,g\rangle},$$
...
This makes regularisation tricky.  The goal is to represent a state $|ψ\rangle$ as a superposition of coherent states, $|ψ\rangle=|A\rangle c$.  We want the coefficients $c$ to correspond to a phase space function $g$ that is nearly analytic.  If the amplitudes $|A\rangle$ formed a regular grid , it would be obvious enough when $c$ was sampling an analytic function.  On an irregular grid, we would expect the coefficients to be larger where the grid is sparse, and smaller where it is dense.  It would be good to know if irregular coefficients are the result of sampling a smooth function on an irregular grid, or of sampling a wildly oscillating function.

The obvious way to attempt this is to factor the coefficients as $|ψ\rangle=|A\rangle Wc'$, where $c'_i$ are samples that approximate $g(α_i)$, and $W$ is a matrix of weights that adapts the samples to the irregular grid.  In fact, we can do this exactly.  If we set $W=(\langle A|A\rangle)^{-1}$, then $c'=\langle A|ψ\rangle$.  We could try seeking coefficients $c$ such that the norm of $\langle A|A\rangle c$ is minimal.
...
There is another way to derive coefficients of the constant function, which might be more stable.  Suppose that $D$ is a subset of the real line, large enough that the tails of the gaussians outside it are neglibible.  Then
$$E=\|f'-1\|=\int_D\left(∑_ic_iu_i(x)-1)²=∑_{ij}c_ic_j\int_D u_iu_j-2∑_ic_i\int_D u_i+\mu(D),$$
where $μ$ is the integration measure.  The derivative of this with respect to $c_i$ is
$${∂E\over ∂c_i}=2∑_{j}c_j\int u_iu_j-2\int u_i,$$
at which point we can expand the domain $D$ to the entire real line.  The condition for stationarity, and presumably best fit, is $Ac=b$, where $a_{ij}=\int u_iu_j$, and $b_i=\int u_i$.  It will be interesting to see what weights come out of this method.
...
%% convergence to the initial state
