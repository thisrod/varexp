...
Let's compute some singular values and vectors.  (And the the wrong answer—try again.)  Suppose that $χ$ has circular symmetry, with $χ(z)=χ(|z|)$.  We can rewrite this
...
\topinsert \hbox{\XeTeXpicfile rsv2.pdf width 0.5\hsize
\XeTeXpicfile rsv3.pdf width 0.5\hsize}
\XeTeXpicfile rsv4.pdf width 0.5\hsize\hfil\break
The right singular functions $v_i$ for the truncated expansion operator $\Gx$, where phase spaces is truncated to a square about the origin, whose size varies.  These are functions of a complex variable: the brightness of the plot indicates modulus, and the color indicates phase.\endinsert
