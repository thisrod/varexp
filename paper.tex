...
The Gaussian wave packet is a ubiquitous idea in quantum mechanics.  Its discovery was inevitable: people were provoked by understanding that observables are essentially uncertain to study physical states in which they are normally distributed.  The Gaussian wavepacket also crops up as the quantum state which, placed in a quadratic potential, best approximates the harmonic oscillations of classical mechanics.  The potential sets a natural width for the packet, which causes its shape to stay constant as it moves.  This width is also characterised by equipartition of energy and the uncertainty principle.  Wavepackets with this natural width are called coherent states.

Wave packets have been a productive tool.  The simplest mechanical systems allow Schrödinger's law of motion to be solved in closed form, but most quantum problems must be solved approximately.  This can be done variationally, specifying a class of approximants that can be manipulated exactly, then finding one of them that most nearly solves the problem.  If the problem is to find a wave function $|ψ(t)〉$ that evolves according to a Hamiltonian $H$, the quality of approximation can be measured by comparing the left and right sides of Schrödinger's equation, $i\hbar{d|ψ〉\over dt}$ and $H|ψ〉$.  These are both vectors in Hilbert space, and can be compared by the norm of their difference.  Gaussian wave packets have often been used as variational approximants, because they are defined by their width and their central position and momentum, useful properties to determine for any quantum state.

The invention of the laser in 1963 called for a quantum theory of intense radiation.  The difficulty of analysing a laser beam stems from the size of its quantum description, up to $10^{18}$ photons distributed across thousands of modes.  This was made managable by Glauber and Sudarshan, who showed that any quantum state can be expanded as a superposition of coherent states.  Laser light is very coherent, so these expansions often represent it efficiently, converging with a reasonable number of states in the superposition.  Moyal(?) developed an efficient way to calculate dynamics, leading to a family of phase space Monte-Carlo methods.  When very coherent matter, in the form of atomic Bose-Einstein condensates, became easy to access in the lab, phase space methods were sucessfully applied to calculate its dynamics.  They have since been applied to other forms of condensed matter.  They have solved many problems, but are limited by intractable convergence issues.
...
One modification would be to use a norm derived from lower order moments of the creation and annihilation operators\cite{Drummond-private}.  In this case, there would not be a direct link between the variational parameters and a state in Hilbert space.  Instead, the superposition of coherent states would be a way to record moments of the system, which would be propagated so that the rates of change of the moments had the expected values.  It is unclear why recording the moments indirectly, by an ensemble of coherent amplitudes, would be better than recording the moments themselves.  Several methods already exist for propagating the moments of a quantum optical system
: these usually assume that some higher-order moments factor in terms of lower-order ones, in order to obtain a system of ordinary differential equations for a finite set of moments.  This assumption is at least explicit, and some equivalent assumption would have to be made implicitly if the moments were derived from an ensemble of coherent amplitudes.  It is unclear why this would be a better approximation to quantum mechanics than the differential equations obtained by assuming the moments factor.


\beginsection{Coherent states}

TODO: Define “phase space” at some point.
...
$$θ=|β-α|+O(|β-α|²).$$
This has a geometric analogy.  The normalised kets form a hypersphere in ket space.  A two-dimensional manifold of normalised kets, such as the coherent states, must be locally similar to the unit sphere.  It turns out that phase space is mapped into ket space without local distortion: the relation between a coherent ket and its neighbours is as if phase space were the surface of a sphere.  This is only the case when the angles between vectors are small; for example, it is misleading in regard to the relation between $|α〉$ and $∂_α|α〉$.
...
Equation~\csip\ has a consequence for expansions.  Suppose a coherent state $|β〉$ is expanded in the form in Equation~\Ad, with components such that $|β-α_r|>δ$, and error $‖|β〉-|ψ〉‖<ε$.  Then $|〈β|ψ〉|>1-ε$, but $|〈β|ψ〉|=|∑〈β|α_r〉d_r|≤e^{-{δ²\over 2}}∑|d_r|$.  Therefore, as the expanded state moves away from the components, the coefficients must grow as $e^{δ²\over 2}$.  The error condition requires that $‖|ψ〉‖<1+ε$, so these components must almost entirely cancel each other out.  This bodes badly for the numerical stability of such expansions: the details are explored below.


\beginsection{Frames}

TODO: say that the default norm in ${\bf C}^n$ is the 2-norm.

In order to support numerical methods, the expansion $|ψ〉=\A d$ needs to be numerically stable.  This means, most obviously, that changing $d$ by a small amount should cause a small change in $\A d$.  This is clearly the case.  The coherent states that compose $\A$ are normalised, so clearly $‖\A d‖≤R‖d‖$, whence $‖\A‖≤R$.

TODO: tighten this bound (can we get $‖\A‖≤\sqrt{R}$?)

The opposite condition, that a small change in $|ψ〉$ should cause a small change in $d$, is trickier.  Finite sets of coherent states are linearly independent in the mathematical sense, so the equation $\A d₀=0$ has no solution, and the operator $\A$ is invertible.  This is not true in the numerical sense: given a typical set of amplitudes $\{α_r\}$, there will be vectors $d₀$ such that $\A d₀$ lies within rounding error of zero.  We can add any multiple of $d₀$ to $d$ without changing the computed $|ψ〉$ at all, so the problem of finding $d$ is utterly unstable.  The best we can hope for is to find a specific expansion that is numerically stable.

As a first step, we can require that the components $〈A|ψ〉$ are stable.  The conventional way to formulate that is to require there are finite constants $P$ and $Q$ such that
$$ P‖|ψ〉‖²≤‖〈A|ψ〉‖²≤Q‖|ψ〉‖²,$$
for all kets $|ψ〉$.  In this case, the set comprising kets $|α_r〉$ is known as a \define{frame}.  Strictly, the mapping $〈A|$ from kets to components is called the frame operator; this paper will often refer to $\A$ as a frame, meaning that its columns form a frame.  The constant $P$ is called the lower frame bound, and $Q$ the upper frame bound.  It will turn out that, in order to form stable expansions, these bounds ought to be close.  The ideal case, where they are equal, is known as a tight frame.

There is a complication that we'll ignore for now.  The state $|ψ〉$ lies in an infinite dimensional Hilbert space, in which there are many directions that are orthogonal to every $|α_r〉$.  When $|ψ〉$ points along such a direction, $〈A|ψ〉=0$.  So no finite set of kets is a frame for Hilbert space, but only for some subspace.  We will discuss this in detail below.

It can be shown that if $\A$ is a frame, the operator $\A〈A|$ is invertible\cite{1992-Daubechies-Ten}.  In this case, the set of kets
$$|F〉=(\A〈A|)^{-1}|A〉=\pmatrix{(\A〈A|)^{-1}|a₁〉&⋯&(\A〈A|)^{-1}|a_R〉}
	=\pmatrix{|φ₁〉&⋯&|φ_R〉}
$$
is also a frame, called the dual frame of $|A〉$.  These kets have the property that $|A〉〈F|=|F〉〈A|=1$, the identity operator on the space spanned by the frame.  In other words, the components of $|ψ〉$ along the directions of the dual frame $|F〉$ are the coefficients of an expansion over the frame $|A〉$, and visa-versa.  If $|α_r〉$ formed an orthonormal basis, we would have $|F〉=|A〉$, and $|A〉〈A|=1$.  In this sense, frames are a generalisation of orthonormal bases.  For a tight frame, $|F〉={1\over P}|A〉$, and the analogy is especially close.
...
\beginsection{Phase space as a Gabor frame}
...
\beginsection{The discrete variational frame}

In the variational problem, the derivative $-iH|ψ(z)〉$ is expanded over the frame $|Dψ(z)〉$.  For the coherent state approximant of Equation~?,
$$ |Dψ(z)〉=\pmatrix{1 & a†}|ψ(z)〉.$$
It follows that
$$ 〈∂₁ψ|∂₁ψ〉=〈ψ|ψ〉=e^{2\Re φ+|α|}
	\qquad 〈∂₂ψ|∂₂ψ〉=(1+|α|²)〈ψ|ψ〉,$$
and
$$\qquad 〈∂₁ψ|∂₂ψ〉=e^{2\Re φ+|α|}〈α|a†|α〉=α*〈ψ|ψ〉.$$
The angle $θ$ between $|∂₁ψ〉$ and $|∂₂ψ〉$, such that
$$ |〈∂₁ψ|∂₂ψ〉|=‖|∂₁ψ〉‖‖|∂₂ψ〉‖\cos θ,$$
satisfies $\cot θ=|α|$.  The partial derivatives are orthogonal for the vacuum state $α=0$, but become nearly parallel as $|α|$ increases.  This will increase the condition number of $|Dψ〉$.


\beginsection{Regularisation of the discrete problems}
%% Picard plots
