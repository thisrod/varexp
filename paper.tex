...
When the first quantum physicists understood that measurements are essentially uncertain, they naturally wondered what conditions would produce the normal law of probability, and discovered gaussian wave packets.  Once they got sick of the free particle and turned to the harmonic oscillator, they would have know to look for its ground state, and how it oscillated in the classical limit.  Gaussian wave packets were the answer to both questions.  Now, with their width fixed so that the action of a cycle was $\hbar$, they became coherent states.

When quantum mechanics was applied to more complicated systems, the equations of motion could not be solved in closed form.  Several approaches were used to approximate them: one of the most versatile was the variational method, in which a class of approximants was specified that could be treated exactly, and it was determined which of them most nearly solved the problem.  The most obvious application was in finding ground states, where the best solution was the one with the lowest energy.  Variational methods were also applied to quantum dynamics, by seeking approximants for which the difference between the two sides of Schrödinger's equation had the least norm in Hilbert space.  Gaussian wavepackets were an obvious choice for the class of approximants.  They are distinguished by their central position and momentum, and their scale: useful things to know about the solution to any problem.  Also, the gaussian packets and their derivatives are easy to manipulate algebraically.
...
In the 1970s, Heller put the two ideas together, and proposed simulating quantum dynamics variationally, using superpositions of coherent states as the approximant.  This is an appealing approach.  Every quantum state can be represented arbitrarily well as the number of components in the superposition becomes large.  The experience of quantum optics shows that many systems can be approximated by a reasonable number of components, and variational dynamics should not have the covergence problems the Monte-Carlo methods do.  There have been several attempts to apply this numerically.  In all cases, numerical instability has prevented the superposition being expanded to the point of convergence.
...
The accuracy of the pertubation method is shown below.
